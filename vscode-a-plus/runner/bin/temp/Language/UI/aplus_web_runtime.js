...
(function () {
        ...
        while (i < xml.length) {
            if (xml[i] === '<') {
                if (i + 1 < xml.length && xml[i + 1] === '/') {
                    ...
                    // Find closing > while respecting quotes
                    let te = i + 1;
                    let inStrTag = false, sqTag = '"';
                    while (te < xml.length) {
                        let ct = xml[te];
                        if (inStrTag) { if (ct === sqTag) inStrTag = false; te++; continue; }
                        if (ct === '"' || ct === "'") { inStrTag = true; sqTag = ct; te++; continue; }
                        if (ct === '>') break;
                        te++;
                    }
                    let tagEnd = te;
                    ...
                    let tagName = parts[0];
                    let attrs = {};
                    // Parse attributes manually to handle arrow functions
                    let attrStr = tagContent.substring(tagName.length);
                    let attrRegex = /(\w[\w.-]*)\s*=\s*("[^"]*"|'[^']*'|[^\s>]+)/g;
                    let m;
                ...
                while (i < code.length) {
                    ...
                    if (c === '<') {
                        if (i + 1 < code.length && code[i + 1] === '/') {
                            ...
                            // Find closing > while respecting quotes
                            let te = i + 1;
                            while (te < code.length && code[te] !== '>') te++;
                            i = te + 1;
                            if (depth <= 0) break;
                        } else if (i + 1 < code.length && /[_a-zA-Z\u0600-\u06FF]/.test(code[i + 1])) {
                            depth++;
                            // Find closing > while respecting quotes
                            let te = i + 1;
                            let inStrTag = false, sqTag = '"';
                            ...
                            let tc = code.substring(i + 1, te);
                            if (tc.endsWith('/') || tc.trim().endsWith('/')) depth--;
                            i = te + 1;
                            if (depth <= 0) break;
                        } else i++;
                    } else i++;
    ...
    class Parser {
        ...
        skipSpawn() {
            this.eat(TT.SPAWN);
            if (this.current() && this.current().type === TT.FUNCTION) this.eat(TT.FUNCTION);
            this.eatName();
            this.eat(TT.LPAREN);
            if (this.current() && this.current().type !== TT.RPAREN) {
                this.eatName();
    ...
    // HTML tag mapping: A+ class names -> HTML tag names
    const htmlTagMap = {
        'button': 'button', 'buttonrepeat': 'button', 'buttontoggle': 'button',
        'checkbox': 'input', 'radiobutton': 'input',
        'textbox': 'input', 'passwordbox': 'input', 'richtextbox': 'textarea',
        'textblock': 'span', 'label': 'label',
        'image': 'img', 'mediaelement': 'video', 'video': 'video', 'sound': 'audio',
        'listbox': 'select', 'combobox': 'select',
        'menu': 'nav', 'menuitem': 'a', 'contextmenu': 'menu',
        'tabcontrol': 'div', 'tabitem': 'div',
        'slider': 'input', 'progressbar': 'progress',
        'datepicker': 'input', 'calendar': 'input',
        'scrollviewer': 'div', 'border': 'div',
        'stackpanel': 'div', 'wrappanel': 'div', 'dockpanel': 'div',
        'grid': 'div', 'canvas': 'div', 'uniformgrid': 'div',
        'viewbox': 'div', 'rectangle': 'div', 'ellipse': 'div',
        'line': 'div', 'polygon': 'div', 'polyline': 'div', 'path': 'svg',
        'window': 'div',
    };

    // HTML attribute mapping: A+ properties -> HTML attributes/properties
    const htmlAttrMap = {
        'content': 'textContent', 'text': 'textContent',
        'width': 'width', 'height': 'height',
        'margin': 'margin', 'padding': 'padding',
        'background': 'backgroundColor', 'foreground': 'color',
        'fontfamily': 'fontFamily', 'fontsize': 'fontSize',
        'fontweight': 'fontWeight', 'fontstyle': 'fontStyle',
        'name': 'id', 'id': 'id',
        'src': 'src', 'source': 'src', 'href': 'href', 'alt': 'alt',
        'placeholder': 'placeholder', 'value': 'value',
        'checked': 'checked', 'disabled': 'disabled', 'readonly': 'readOnly',
        'min': 'min', 'max': 'max', 'step': 'step',
        'rows': 'rows', 'cols': 'cols',
        'horizontalalignment': 'textAlign', 'verticalalignment': 'verticalAlign',
        'visibility': 'visibility', 'opacity': 'opacity',
        'position': 'currentTime',
    };

    // HTML event mapping: A+ events -> DOM events
    const htmlEventMap = {
        'click': 'click', 'loaded': 'load',
        'mouseenter': 'mouseenter', 'mouseleave': 'mouseleave',
        'mousedown': 'mousedown', 'mouseup': 'mouseup', 'mousemove': 'mousemove',
        'keydown': 'keydown', 'keyup': 'keyup',
        'gotfocus': 'focus', 'lostfocus': 'blur',
        'changed': 'input', 'selectionchanged': 'change',
        'valuechanged': 'input', 'sizechanged': 'resize',
        'checked': 'change', 'unchecked': 'change',
    };

    function isHtmlTag(tag) { return tag.toLowerCase() in htmlTagMap; }
    function toHtmlTag(tag) { return htmlTagMap[tag.toLowerCase()] || tag.toLowerCase(); }
    function toHtmlAttr(attr) { return htmlAttrMap[attr.toLowerCase()] || attr.toLowerCase(); }
    function toHtmlEvent(evt) { return htmlEventMap[evt.toLowerCase()] || evt.toLowerCase(); }

    // WebUIElement — wraps a real DOM element for the web runtime
    class WebUIElement {
        constructor(tag) {
            this._tag = tag;
            this._fields = {};
            ...
            this._methods = {};
            this._dom = null;
            // Create real DOM element if running in browser
            if (typeof document !== 'undefined' && isHtmlTag(tag)) {
                this._dom = document.createElement(toHtmlTag(tag));
                if (tag.toLowerCase() === 'checkbox' || tag.toLowerCase() === 'radiobutton') {
                    this._dom.type = tag.toLowerCase() === 'radiobutton' ? 'radio' : 'checkbox';
                }
            }
        }
        ...
        setField(name, val) {
            ...
            if (this._dom) {
                const htmlEvt = toHtmlEvent(name);
                const isEvent = htmlEventMap.hasOwnProperty(name.toLowerCase());
                if (isEvent) {
                    // It's an event
                    if (typeof val === 'function') {
                        this._dom.addEventListener(htmlEvt, val);
                    }
                } else {
                    // It's a property
                    const htmlName = toHtmlAttr(name);
                    try {
                        if (htmlName === 'checked' || htmlName === 'disabled' || htmlName === 'readOnly') {
                            this._dom[htmlName] = !!val;
                        } else {
                            this._dom[htmlName] = val;
                        }
                    } catch (e) { /* ignore unsupported props */ }
                }
            }
        }
        ...
        getField(name) {
            if (this._dom) {
                const htmlName = toHtmlAttr(name);
                try { return this._dom[htmlName]; } catch (e) { /* ignore */ }
            }
        ...
        bindEvent(eventName, handler) {
            this._methods[eventName] = handler;
            if (this._dom) {
                const htmlEvt = toHtmlEvent(eventName);
                if (typeof handler === 'function') {
                    this._dom.addEventListener(htmlEvt, handler);
                }
            }
        }
    ...
    class Interpreter {
        ...
        execMemberSet(stmt) {
            ...
            if (typeof obj === 'object' && obj !== null) {
                if (obj._fields !== undefined && typeof obj.setField === 'function') {
                    obj.setField(stmt.member, val);
                } else if (obj._fields) { obj._fields[stmt.member] = val; }
        ...
        execArrowBind(stmt) {
            ...
            try {
                ...
                if (stmt.object === '__root') {
                    obj = new WebUIElement('div');
                    if (typeof document !== 'undefined' && obj._dom) {
                        const container = document.getElementById('output') || document.body;
                        obj._dom.id = '__aplus_root';
                        container.appendChild(obj._dom);
                    }
            ...
            if (typeof obj === 'object' && obj !== null) {
                if (typeof obj.bindEvent === 'function') {
                    obj.bindEvent(stmt.event, handler);
                } else if (obj._methods) obj._methods[stmt.event] = handler;
            ...
            if (node.name === '__root') {
                try { return this.env.getVar('__root'); }
                    ...
                    if (typeof document !== 'undefined') {
                        const container = document.getElementById('output') || document.body;
                        root = new WebUIElement('div');
                        root._fields['id'] = '__aplus_root';
                        if (root._dom) {
                            root._dom.id = '__aplus_root';
                            root._dom.style.fontFamily = 'sans-serif';
                            container.appendChild(root._dom);
                        }
                    } else {
                        root = new WebUIElement('Root');
                    }
        ...
        evalCall(node) {
            ...
            if (node.callee.type === 'dot') {
                ...
                if (methodName === 'stop' && obj && obj._dom && typeof obj._dom.pause === 'function') {
                    obj._dom.pause();
                    obj._dom.currentTime = 0;
                    return null;
                }
        ...
        evalDot(node) {
            ...
            if (typeof obj === 'object') {
                ...
                if (typeof obj.getField === 'function') return obj.getField(node.member);
